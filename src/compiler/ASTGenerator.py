# following 6 lines were necessary to import type and node specific files
import os
import sys
lib_path_nodes = os.path.abspath(os.path.join('.', 'nodes'))
lib_path_types = os.path.abspath(os.path.join('.', 'types'))
sys.path.append(lib_path_nodes)
sys.path.append(lib_path_types)
...
import Function
import ParameterDeclarationList
import Type
import TypeSpecifier
import CompoundStatement
import BreakStatement
import ContinueStatement
import ReturnStatement
import WriteIntStatement
import Identifier
import ParameterDeclaration
import ParameterList
import VariableIdentifier
import IfElseStatement
import IfStatement
...
class ASTGenerator(SmallCVisitor):
    ...
    def visitFunction_definition(self, parsetree:SmallCParser.Function_definitionContext):
        ...
        if parsetree.param_decl_list() is None:
            parameter_list = ParameterDeclarationList.ParameterDeclarationList(self.ast, [], False)
        else:
    ...
    def visitCompound_stmt(self, parsetree:SmallCParser.Compound_stmtContext, functionBody):
        ...
        stmt = CompoundStatement.CompoundStatement(self.ast, var_decls, statements)
    ...
    def visitVariable_id(self, parsetree:SmallCParser.Variable_idContext):
        ...
        if parsetree.identifier().array_definition() is None:
            ...
            array_size = int(parsetree.identifier().array_definition().INTEGER().getText())
    ...
    def visitFor_stmt(self, parsetree:SmallCParser.For_stmtContext):
        ...
        statement = self.visit(parsetree.stmt())
        for_stmt = ForStatement.ForStatement(self.ast, var_decl, condition, update, statement)
    ...
    def visitAssignment(self, parsetree:SmallCParser.AssignmentContext):
        ...
        if parsetree.identifier().array_definition() is None:
            ...
            index = int(parsetree.identifier().array_definition().INTEGER().getText())
