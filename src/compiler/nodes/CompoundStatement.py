...
class CompoundStatement(Statement, ControlStructure):
    ...
    def generateCode(self, out):
        if isinstance(self.parent, Function):
            ...
            self.writeInstruction("cup 0 compound_" + str(self.label_id) + "_body", out)
            self.writeInstruction("dpl i", out)
            self.writeInstruction("ldc i 0", out)
            self.writeInstruction("neq i", out)
            self.writeInstruction("fjp compound_" + str(self.label_id) + "_end", out)
            self.writeInstruction("dpl i", out)
            ...
            self.writeInstruction("compound_" + str(self.label_id) + "_body:", out)
            ...
            self.writeInstruction("compound_" + str(self.label_id) + "_continue:", out)
            self.writeInstruction("ldc i 0", out)
            self.writeInstruction("str i 0 0", out)
            self.writeInstruction("retf", out)
            self.writeInstruction("compound_" + str(self.label_id) + "_return:", out)
            self.writeInstruction("ldc i 1", out)
            self.writeInstruction("str i 0 0", out)
            self.writeInstruction("retf", out)
            self.writeInstruction("compound_" + str(self.label_id) + "_break:", out)
            self.writeInstruction("ldc i 2", out)
            self.writeInstruction("str i 0 0", out)
            self.writeInstruction("retf", out)
            self.writeInstruction("compound_" + str(self.label_id) + "_end:", out)
